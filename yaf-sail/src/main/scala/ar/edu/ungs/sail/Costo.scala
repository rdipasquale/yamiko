...
object Costo extends Serializable{
  ...
  def calcCostoNavegacionEst(u:Nodo,v:Nodo,metrosPorCelda:Int,nodosPorCelda:Int,valores:List[EstadoEscenarioViento],vmg:VMG):Float={
      ...
      if (Math.signum(v.getY-u.getY)>=0) 
        ...
        anguloNavegacion=180d}
         ...
         anguloNavegacion=90d+Math.abs(Math.toDegrees(Math.atan((v.getY().doubleValue()-u.getY().doubleValue())/(v.getX().doubleValue()-u.getX().doubleValue()))))
           ...
           anguloNavegacion=270+Math.abs((Math.toDegrees(Math.atan((v.getY().doubleValue()-u.getY().doubleValue())/(v.getX().doubleValue()-u.getX().doubleValue())))))
    val anguloNormalizado=(540-vientos(0).getAngulo()+Math.round(anguloNavegacion))%360 
    val velocidadMaxima=vmg.getSpeed(anguloNormalizado.toInt, vientos(0).getVelocidad())
