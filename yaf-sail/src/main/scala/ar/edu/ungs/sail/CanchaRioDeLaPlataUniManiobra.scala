...
@SerialVersionUID(1L)
class CanchaRioDeLaPlataUniManiobra(_dimension:Int, _nodosPorCelda:Int, _metrosPorLadoCelda:Int,nodoInicial:Nodo,nodoFinal:Nodo,islas:List[(Int,Int)],viento:List[EstadoEscenarioViento]) extends Cancha
{
  ...
  for (x <- 0 to (dimension*(nodosPorCelda-1)))
    for (y <- 0 to (dimension*(nodosPorCelda-1)))      
      if(x==0 || x%(nodosPorCelda-1)==0 || y==0 || y%(nodosPorCelda-1)==0 )
          MANIOBRAS.values.take(1).foreach(m => nodos+=new Nodo(x,y,"("+x+")"+"("+y+")-"+m,armarCuadrantes(x, y),m))
        
	// Arcos navegacion
  for (x <- 0 to dimension-1)
    for (y <- 0 to dimension-1)
        MANIOBRAS.values.take(1).foreach(m => ((nodos.filter(p=>p.getCuadrante().contains((x,y)) && p.getManiobra().equals(m)).combinations(2)).foreach(f=>{
  ...
  MANIOBRAS.values.take(1).foreach(m => {
