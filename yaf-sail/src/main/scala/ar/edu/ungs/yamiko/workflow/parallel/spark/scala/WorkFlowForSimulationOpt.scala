...
class WorkFlowForSimulationOpt(uriSpark:String, 
                                  ...
                                  barco:VMG,
                                  genes:List[Gene],
                                  translators:Map[Gene, Ribosome[List[(Int,Int)]]],
                                  ...
                                  dimension:Int,
                                  nodosPorCelda:Int, 
                                  metrosPorLadoCelda:Int) extends Serializable{
  ...
  def run( ):Individual[List[(Int,Int)]] =
  {
    ...
    val salida=promedios.zip(resultranking).map(f=>(f._1._1,f._1._2*f._2._2))
		...
		for (p<-0 to po.size()-1)
		  po.getAll()(p).setFitness(salida.find(_._1==po.getAll()(p).getId()).get._2)
    }
    
    new BasicIndividual[List[(Int,Int)]](null,0)
  }
}