...
class SparkParallelFitnessGA[T] (parameter: Parameter[T],isolatedGenerations:Int) extends Serializable{
			  ...
			  populations=populations.map { dp:DistributedPopulation[T] => 
		          ...
		          val descendants=new ListBuffer[Individual[T]]
    			      g+=1
    			        ...
    			        if (g%1==0) Logger.getLogger("file").warn("Generation " + dp.getId() + " - " + g  + " -> Mejor Individuo -> Fitness: " + bestOfGeneration.getFitness() + " - " + bestOfGeneration.getGenotype().getChromosomes()(0).getFullRawRepresentation());
